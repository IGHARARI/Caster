...
import sts.caster.cards.special.IceShard;
import sts.caster.core.CasterMod;
import sts.caster.patches.spellCardType.CasterCardType;

//Gain 1 dex for the turn for each card played.

public class ShatteredStance extends AbstractStance {

	public static final String STANCE_ID = CasterMod.makeID("ShatteredStance");
	private static long sfxId = -1L;

	public ShatteredStance() {
		this.ID = STANCE_ID;
	...
	public void onPlayCard(AbstractCard card) {
		...
		if (card instanceof CasterCard && card.type == CasterCardType.SPELL) {
			AbstractDungeon.actionManager.addToBottom(new MakeTempCardInHandAction(new IceShard()));
		}
	}
